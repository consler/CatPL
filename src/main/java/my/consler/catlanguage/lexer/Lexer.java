package my.consler.catlanguage.lexer;
    ...
    public static List<Token> tokenize(String input)
    {
        ...
        for (String line : lines)
        {
            ...
            while (matcher.find())
            {
                ...
                {
                    tokens.add(new Token(TokenType.KEYWORD, token_value));

                }
                else if (isEvent(token_value))
                {
                    tokens.add(new Token(TokenType.EVENT, token_value));

                }
                else if (token_value.matches(IDENTIFIER))
                {
                    tokens.add(new Token(TokenType.IDENTIFIER, token_value));

                }
                else if (token_value.matches(INTEGER))
                {
                    tokens.add(new Token(TokenType.INTEGER, token_value));

                }
                else if (token_value.matches(STRING))
                {
                    tokens.add(new Token(TokenType.STRING, token_value.substring(1, token_value.length() - 1)));

                }
                else if (token_value.matches(SYMBOL))
                {
                    tokens.add(new Token(TokenType.SYMBOL, token_value));
